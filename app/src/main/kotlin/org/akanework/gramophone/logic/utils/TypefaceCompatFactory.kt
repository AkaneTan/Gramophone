...
class TypefaceCompatFactory private constructor(context: Context?) : Factory2 {
	...
	override fun onCreateView(parent: View?, name: String, context: Context, attrs: AttributeSet): View? {
		...
		if (result is TextView) {
			...
			if (appearance != null) {
				fontWeight = appearance.getInt(R.styleable.TextAppearance_android_textFontWeight, -1)
				appearance.recycle()
			}
			a = theme.obtainStyledAttributes(attrs, R.styleable.TextAppearance, 0, 0)
			if (a.hasValue(R.styleable.TextAppearance_android_textFontWeight)) {
				fontWeight = a.getInt(R.styleable.TextAppearance_android_textFontWeight, -1)
			}
			a.recycle()
			if (fontWeight != -1) {
				val tf = TypefaceCompat.create(result.context, result.typeface, fontWeight, result.typeface.isItalic)
